...
idiomas = {
    "Brasil 🇧🇷": {
        ...
        "trend_header": "Legenda de Tendência", "trend_corpo": "📈 Ascendente (Alta Procura)\n➡️ Flat (Estável)\n📉 Decrescente (Baixa)",
    ...
    "Portugal 🇵🇹": {
        ...
        "trend_header": "Legenda de Tendência", "trend_corpo": "📈 Ascendente\n➡️ Estável\n📉 Decrescente",
    ...
    "USA 🇺🇸": {
        ...
        "ajuda_header": "Risk Legend", "ajuda_corpo": "✅ Winning\n⚠️ Expensive\n🟥 Alert",
        "trend_header": "Trend Legend", "trend_corpo": "📈 Rising\n➡️ Stable\n📉 Falling",
...
with st.sidebar:
    ...
    st.divider()
    st.header(t["ajuda_header"]); st.info(t["ajuda_corpo"])
    st.header(t["trend_header"]); st.info(t["trend_corpo"])
...
if not st.session_state.api_key:
    ...
    if "Bling" in fonte:
        ...
        with c_bl: token = st.text_input("Token Bling:", type="password")
        if st.button("📥 Importar"):
            try:
                r = requests.get("https://bling.com.br", headers={"Authorization": f"Bearer {token}"})
    ...
    if not df_base.empty:
        ...
        with cp2: markup = st.number_input("% Markup", 0, 500, 70) / 100
        if st.button(t["btn_analisar"]):
            with st.spinner('Processando...'):
                ...
                df['Preço Sugerido'] = df.apply(lambda x: round(x['Mercado']*0.98, 2) if (x['Custo']*(1+markup)) > x['Mercado'] else round(x['Custo']*(1+markup), 2), axis=1)
                df['Lucro Total'] = round(((df['Preço Sugerido']*(1-imposto)) - df['Custo']) * df['Qtde'], 2)
                df['Status'] = df.apply(lambda x: "🟥" if x['Mercado'] < x['Custo'] else ("⚠️" if (x['Custo']*(1+markup)) > x['Mercado'] else "✅"), axis=1)
